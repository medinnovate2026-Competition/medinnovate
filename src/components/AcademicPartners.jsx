        ...
        {partners.length === 0 ? (
          ...
          <div className="mx-auto mt-16 grid max-w-6xl gap-7 sm:grid-cols-2 lg:grid-cols-4">
            {partners.map((partner, index) => (
              ...
              >
                <div className="grid h-20 w-20 place-items-center rounded-3xl border border-violet-100 bg-white shadow-sm transition group-hover:scale-105">
                  {partner.logo_url ? (
                ...
                {partner.country && <p className="mt-2 text-sm font-black uppercase tracking-[0.16em] text-[#EC4899]">{partner.country}</p>}
                <p className="mt-3 flex-1 text-base leading-7 text-slate-600">{partner.description}</p>
                {partner.website && (
                  <a
